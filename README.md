...
The `approve` command performs a transactional approval write. It flushes a same-directory temporary file before atomically replacing `.codev.md`; interruption leaves either the complete previous state or the complete approved state. The atomic backup is compared with the validated snapshot, and bounded atomic recovery follows any superseding edits instead of deleting them. If continuous writers prevent recovery from settling, the latest recovery file is retained and approval fails. On Unix, the backup is also used to republish the approved bytes with the latest permission and ACL metadata. Successful approval preserves the supported original encoding and BOM, newline style, and non-field content. New unmarked state files and the default template remain UTF-8 without BOM.
